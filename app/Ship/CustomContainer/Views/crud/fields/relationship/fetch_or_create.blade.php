        ...
        if (!isset($inlineCreate)) {
            ...
            if (!is_bool($field['inline_create']['include_main_form_fields'])) {
                if (is_array($field['inline_create']['include_main_form_fields'])) {
                    ...
                    $arrayed_field = [$field['inline_create']['include_main_form_fields']];
...
@if ($activeInlineCreate)
    @include('customcontainer::crud.fields.relationship.inline_create_button', ['field' => $field])
@endif
<select name="{{ $field['name'] . ($field['multiple'] ? '[]' : '') }}"
    data-field-is-inline="{{ var_export($inlineCreate ?? false) }}" data-original-name="{{ $field['name'] }}"
    style="width: 100%" data-force-select="{{ var_export($field['inline_create']['force_select']) }}"
    data-init-function="bpFieldInitFetchOrCreateElement" data-allows-null="{{ var_export($field['allows_null']) }}"
    data-dependencies="{{ isset($field['dependencies']) ? json_encode(Arr::wrap($field['dependencies'])) : json_encode([]) }}"
    data-model-local-key="{{ $crud->model->getKeyName() }}" data-placeholder="{{ $field['placeholder'] }}"
    data-data-source="{{ $field['data_source'] }}" data-method="{{ $field['method'] ?? 'POST' }}"
    data-minimum-input-length="{{ $field['minimum_input_length'] }}" data-field-attribute="{{ $field['attribute'] }}"
    data-connected-entity-key-name="{{ $connected_entity_key_name }}"
    data-include-all-form-fields="{{ var_export($field['include_all_form_fields']) }}"
    data-current-value="{{ $field['value'] }}" data-field-ajax="{{ var_export($field['ajax']) }}"
    ...
    data-ajax-delay="{{ $field['delay'] }}" data-language="{{ str_replace('_', '-', app()->getLocale()) }}"
    @if ($activeInlineCreate) @include('customcontainer::crud.fields.relationship.field_attributes') @endif @include('customcontainer::crud.fields.inc.attributes', [
        'default_class' => 'form-control select2_field',
    ])
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        ...
        @endif
        <script>
            document.styleSheets[0].addRule('.select2-selection__clear::after',
            ...
            function setupInlineCreateButtons(element) {
                ...
                var $includeMainFormFields = element.attr('data-include-main-form-fields') == 'false' ? false : (element.attr(
                    'data-include-main-form-fields') == 'true' ? true : element.attr('data-include-main-form-fields'));
                ...
                $modalSaveButton.on('click', function() {
                    ...
                    $.ajax({
                        ...
                        error: function(result) {
                            ...
                            new Noty({
                                type: "error",
                                text: '<strong>{{ trans('backpack::crud.related_entry_created_error') }}</strong><br> ' +
                                    message,
                            }).show();
            ...
            function bpFieldInitFetchOrCreateElement(element) {
                ...
                if (!$allows_null && !$item && $selectedOptions == null) {
                    fetchDefaultEntry(element).then(function(result) {
                        $(element).append('<option value="' + result[$modelKey] + '">' + result[$fieldAttribute] +
                            '</option>');
            ...
            if (typeof processItemText !== 'function') {
                function processItemText(item, $fieldAttribute) {
                    ...
                    return typeof $itemField === 'object' && $itemField !== null ?
                        $itemField[$appLang] ? $itemField[$appLang] : $itemField[$appLangFallback] ? $itemField[
                            $appLangFallback] : Object.values($itemField)[0] ? Object.values($itemField)[0] :
                        $emptyTranslation :
                        $itemField;
                }
            }
