...
<div class="card">
    ...
    @include('customcontainer::crud.fields.relationship.show_fields', ['fields' => $crud->getFieldsWithoutATab()])
    </div>
</div>
@else
    @include('customcontainer::crud.fields.relationship.show_fields', ['fields' => $crud->getFieldsWithoutATab()])
    ...
    <div class="nav-tabs-custom {{ $horizontalTabs ? '' : 'row'}}" id="inline_form_tabs">
        ...
        <div class="tab-content p-0 {{$horizontalTabs ? '' : 'col-md-9'}}">
            ...
            <div role="tabpanel" class="tab-pane {{ isset($tabWithError) ? ($tab == $tabWithError ? ' active' : '') : ($k == 0 ? ' active' : '') }}" id="inline_tab_{{ Str::slug($tab, "") }}">
                ...
                <div class="row">
                @include('customcontainer::crud.fields.relationship.show_fields', ['fields' => $crud->getTabFields($tab)])
                </div>
            </div>
            @endforeach
