@php
    ...
    $current_value = $field['value'] ?? ($field['default'] ?? '');
...
<select style="width:100%" name="{{ $field['name'] . ($field['multiple'] ? '[]' : '') }}"
    data-init-function="bpFieldInitFetchElement" data-field-is-inline="{{ var_export($inlineCreate ?? false) }}"
    ...
    data-method="{{ $field['method'] ?? 'POST' }}" data-data-source="{{ $field['data_source'] }}"
    data-field-attribute="{{ $field['attribute'] }}" data-connected-entity-key-name="{{ $connected_entity_key_name }}"
    data-include-all-form-fields="{{ var_export($field['include_all_form_fields']) }}"
    data-current-value="{{ $field['value'] }}" data-app-current-lang="{{ app()->getLocale() }}"
    data-ajax-delay="{{ $field['delay'] }}" data-language="{{ str_replace('_', '-', app()->getLocale()) }}"
    @include('customcontainer::crud.fields.inc.attributes', ['default_class' => 'form-control']) @if ($field['multiple']) multiple @endif>
</select>
            ...
            function bpFieldInitFetchElement(element) {
                ...
                if (!$allows_null && $item === false && $selectedOptions == null) {
                    fetchDefaultEntry(element).then(function(result) {
                        ...
                        $(element).append('<option value="' + $item[$modelKey] + '">' + $item[$fieldAttribute] +
                            '</option>');
