        ...
        switch (gettype($current_value)) {
            ...
            case 'object':
                if (is_subclass_of(get_class($current_value), 'Illuminate\Database\Eloquent\Model')) {
                    ...
                    $current_value = $current_value->pluck($field['attribute'], $connected_entity_key_name);
                }
...
<select style="width:100%" name="{{ $field['name'] . ($field['multiple'] ? '[]' : '') }}"
    ...
    data-current-value="{{ $field['value'] }}" data-field-multiple="{{ var_export($field['multiple']) }}"
    data-language="{{ str_replace('_', '-', app()->getLocale()) }}" @include('customcontainer::crud.fields.inc.attributes', ['default_class' => 'form-control'])
