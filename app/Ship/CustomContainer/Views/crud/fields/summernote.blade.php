...
<textarea name="{{ $field['name'] }}" data-init-function="bpFieldInitSummernoteElement"
    data-options="{{ json_encode($field['options']) }}" @include('customcontainer::crud.fields.inc.attributes', ['default_class' => 'form-control summernote'])>{{ $field['value'] ?? ($field['default'] ?? '') }}</textarea>
