...
<textarea name="{{ $field['name'] }}" data-init-function="bpFieldInitCKEditorElement"
    data-options="{{ trim(json_encode($field['options'])) }}" @include('customcontainer::crud.fields.inc.attributes', ['default_class' => 'form-control'])>{{ $field['value'] ?? ($field['default'] ?? '') }}</textarea>
