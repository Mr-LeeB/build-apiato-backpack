...
<div class="input-group colorpicker-component">
    <input type="text" name="{{ $field['name'] }}" value="{{ $field['value'] ?? ($field['default'] ?? '') }}"
        data-init-function="bpFieldInitColorPickerElement" @include('customcontainer::crud.fields.inc.attributes')>
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        <script type="text/javascript" src="{{ asset('packages/bootstrap-colorpicker/dist/js/bootstrap-colorpicker.min.js') }}">
        </script>
        <script>
