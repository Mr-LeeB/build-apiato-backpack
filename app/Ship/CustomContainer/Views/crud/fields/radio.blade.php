<!-- radio -->
@php
    $optionValue = $field['value'] ?? ($field['default'] ?? '');
...
@if (isset($field['options']) && ($field['options'] = (array) $field['options']))
    ...
    @foreach ($field['options'] as $value => $label)
        <div class="form-check {{ isset($field['inline']) && $field['inline'] ? 'form-check-inline' : '' }}">
            <input type="radio" class="form-check-input" value="{{ $value }}" @include('customcontainer::crud.fields.inc.attributes')>
            <label
                class="{{ isset($field['inline']) && $field['inline'] ? 'radio-inline' : '' }} form-check-label font-weight-normal">{!! $label !!}</label>
        </div>
    @endforeach
