...
trait Search
{
    ...
    */
    ...
    public function getRowViews($entry, $rowNumber = false)
    {
        ...
        //     $row_items[] = \View::make('customcontainer::crud.inc.button_stack', ['stack' => 'line'])
        //         ->with('crud', $this)
        //         ->with('entry', $entry)
        ...
        //     $details_row_button = \View::make('customcontainer::crud.columns.inc.details_row_button')
        //         ->with('crud', $this)
        //         ->with('entry', $entry)
        //         ->with('row_number', $rowNumber)
        //         ->render();
        ...
        $row_items[] = \View::make('customcontainer::crud.buttons.actions')
            ->with('crud', $this)
            ->with('entry', $entry)
            ->with('row_number', $rowNumber)
            ->render();
    ...
    private function getCellViewName($column)
    {
        ...
        if (isset($column['type'])) {
            ...
            return 'customcontainer::crud.columns.' . $column['type'];
        }

        // fallback to text column
        return 'customcontainer::crud.columns.text';
    }
    ...
    {
        if (!view()->exists($view)) {
            $view = 'customcontainer::crud.columns.text'; // fallback to text column
        }
     ...
     * @return array
     */
    public function getEntriesAsJsonForDatatables($entries, $totalRows, $filteredRows, $draw, $startIndex = false)
    {
        $rows = [];
