...
trait Settings
{
    ...
    public function getOperationSetting(string $key, $operation = null)
    {
        ...
        return $this->get($operation . '.' . $key) ?? config('custom.crud.operations.' . $this->getCurrentOperation() . '.' . $key) ?? null;
    }
     ...
     * @param  string|array  $value  True/false depending on success.
     */
    ...
    /**
     * Automatically set values in config file (config/custom/crud)
    ...
    public function loadDefaultOperationSettingsFromConfig($configPath = null)
    {
        ...
        $configPath = $configPath ?? 'custom.crud.operations.' . $operation;
